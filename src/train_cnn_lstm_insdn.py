...
# coding: utf-8
"""
CNN-LSTM IDS Training and Evaluation -- InSDN Dataset

Trains and evaluates the Bayesian-optimized CNN-LSTM intrusion detection
model on the InSDN dataset (Elsayed et al., 2020), benchmarked against
gradient-boosting and single-branch (CNN-only, LSTM-only) baselines.
Includes Optuna TPE hyperparameter search, sensitivity analysis over the
composite objective's penalty weights, and export of thesis-ready result
tables.

This script is a cleaned, execution-equivalent conversion of the original
Kaggle notebook (final-optimized-cnn-lstm-insdn-only.ipynb) used to produce the results reported in the
...
Usage:
    ...
    Kaggle's /kaggle/input mount), then run:
        python train_cnn_lstm_insdn.py
"""
...
# Cell 3: Reproducibility and Kaggle configuration — InSDN only
SEED = 42
random.seed(SEED)
np.random.seed(SEED)
...
def find_insdn_csv_files():
    """Auto-discover all InSDN CSV files from /kaggle/input."""
    import glob as _glob
    all_csvs = sorted(_glob.glob("/kaggle/input/**/*.csv", recursive=True))

    insdn_dirs = set()
    for p in all_csvs:
        if any(t in p.lower() for t in ["insdn", "in-sdn", "in_sdn"]):
            insdn_dirs.add(os.path.dirname(p))

    if insdn_dirs:
        files = sorted({p for p in all_csvs if os.path.dirname(p) in insdn_dirs})
        if files:
            print(f"  [InSDN] Found {len(files)} CSV(s) via directory match:")
            for f in files:
                print(f"    {f}  ({os.path.getsize(f)/1024**2:.1f} MB)")
            return files

    keywords = ("normal_data", "normal-data", "ovs", "metasploit",
                "insdn", "in-sdn", "in_sdn")
    files = sorted({
        p for p in all_csvs
        if any(k in os.path.basename(p).lower() for k in keywords)
        and "ton" not in p.lower()
        and "edgeiiot" not in p.lower()
    })
    if files:
        print(f"  [InSDN] Found {len(files)} CSV(s) via filename keywords:")
        for f in files:
            print(f"    {f}  ({os.path.getsize(f)/1024**2:.1f} MB)")
        return files

    return ["/kaggle/input/CHANGE_ME/Normal_data.csv"]


CONFIG = {
    "DATASET_PATHS":  {"InSDN": find_insdn_csv_files()},
    "TARGET_COLUMNS": {"InSDN": "Label"},
    "RESULTS_DIR": str(KAGGLE_WORKING_ROOT / "thesis_results"),
    "OUTPUT_ZIP":  str(KAGGLE_WORKING_ROOT / "thesis_results.zip"),
    "TEST_SIZE":  0.15,
    "VAL_SIZE":   0.15,
    "TOP_K_FEATURES": 32,
    "TOP_K_FEATURES_BY_DATASET": {"InSDN": 32},
...
print("\nInSDN paths:")
for p in CONFIG["DATASET_PATHS"]["InSDN"]:
    exists = os.path.exists(p)
    print(f"  {'✓' if exists else '✗'} {p}")
if any("CHANGE_ME" in str(p) for p in CONFIG["DATASET_PATHS"]["InSDN"]):
    print("\n⚠ Attach the InSDN dataset via Add Data and re-run this cell.")

# Cell 4: Verify InSDN files are accessible
...
for p in CONFIG["DATASET_PATHS"]["InSDN"]:
    exists = os.path.exists(p)
    size   = f"{os.path.getsize(p)/1024**2:.1f} MB" if exists else "—"
...
def prepare_dataset(dataset_name, file_path):
    ...
    # Support both a single path (string) and a list of paths (multi-file InSDN).
...
# Cell 13: Run all experiments on InSDN
all_studies = {}

_paths   = CONFIG["DATASET_PATHS"]["InSDN"]
_missing = [p for p in _paths if "CHANGE_ME" in str(p) or not os.path.exists(str(p))]
if _missing:
    print("InSDN files not found:")
    for m in _missing:
        print(f"  ✗ {m}")
    print("Attach the dataset via Add Data and re-run.")
else:
    results_df, reports, study, best_params = run_experiments_for_dataset(
        "InSDN", CONFIG["DATASET_PATHS"]["InSDN"]
    )
    all_studies["InSDN"] = study
    final_results = results_df.copy()
    final_results.to_csv(
        os.path.join(CONFIG["RESULTS_DIR"], "INSDN_FINAL_RESULTS.csv"), index=False
    )
    print("\nInSDN results:")
...
# Cell 14: Save thesis-ready summary table
thesis_columns = [
    "dataset",
    "model",
    "accuracy",
...
thesis_table.sort_values(by=["dataset", "f1_macro"], ascending=[True, False], inplace=True)
...
print(f"\nAll results saved inside: {CONFIG['RESULTS_DIR']}")
print("Figures are saved as PDF for LaTeX and PNG for preview according to CONFIG['FIGURE_FORMATS'].")
...
_latex_df.to_latex(
    ...
    caption="InSDN: model performance summary.",
    label="tab:insdn_results",
    column_format="ll" + "r" * (len(_latex_df.columns) - 2),
)
print(f"LaTeX table saved: {_latex_path}")
...
(
    ...
    .set_caption("InSDN results: all models")
    .to_html(_html_path)
)
print(f"Styled HTML table saved: {_html_path}")
display(
    thesis_table.style
    .format({c: "{:.4f}" for c in _num_cols}, na_rep="—")
    .format(_other_fmt, na_rep="—")
    ...
    .set_caption("InSDN — green = best, highlighted = top performer per metric")
)
...
# Cell 15: Export runtime environment, hyperparameter summaries, and appendix tables
...
baseline_setting_frames = []

for dataset_name in CONFIG["DATASET_PATHS"].keys():
    dataset_dir = os.path.join(CONFIG["RESULTS_DIR"], dataset_name)

    class_dist_path = os.path.join(dataset_dir, f"{dataset_name}_class_distribution.csv")
    if os.path.exists(class_dist_path):
        df = pd.read_csv(class_dist_path)
        df.insert(0, "dataset", dataset_name)
        class_distribution_frames.append(df)
    ...
    if os.path.exists(best_params_path):
        ...
        bp_df.insert(0, "dataset", dataset_name)
        best_params_frames.append(bp_df)

    baseline_settings_path = os.path.join(dataset_dir, f"{dataset_name}_baseline_settings.csv")
    if os.path.exists(baseline_settings_path):
        baseline_setting_frames.append(pd.read_csv(baseline_settings_path))

    for filename in os.listdir(dataset_dir):
        if filename.endswith("_classification_report.csv"):
            report_path = os.path.join(dataset_dir, filename)
            ...
            classification_report_frames.append(report_df)

if best_params_frames:
    best_params_summary = pd.concat(best_params_frames, ignore_index=True)
    best_params_summary.to_csv(
        os.path.join(CONFIG["RESULTS_DIR"], "INSDN_BEST_PARAMS.csv"),
        index=False
    )
    print("\nCombined best hyperparameters:")
    display(best_params_summary)

if class_distribution_frames:
    all_class_distributions = pd.concat(class_distribution_frames, ignore_index=True)
    all_class_distributions.to_csv(
        os.path.join(CONFIG["RESULTS_DIR"], "INSDN_CLASS_DISTRIBUTIONS.csv"),
        index=False
    )
    print("\nCombined class distributions:")
    display(all_class_distributions.head(20))

if baseline_setting_frames:
    all_baseline_settings = pd.concat(baseline_setting_frames, ignore_index=True)
    all_baseline_settings.to_csv(
        os.path.join(CONFIG["RESULTS_DIR"], "INSDN_BASELINE_SETTINGS.csv"),
        index=False
    )
    print("\nCombined baseline settings saved: ALL_DATASETS_BASELINE_SETTINGS.csv")
    display(all_baseline_settings)

if classification_report_frames:
    all_classification_reports = pd.concat(classification_report_frames, ignore_index=True)
    all_classification_reports.to_csv(
        os.path.join(CONFIG["RESULTS_DIR"], "INSDN_CLASSIFICATION_REPORTS.csv"),
        index=False
    )
    print("\nCombined classification report appendix saved.")

print("\nAdditional Chapter Four artifacts exported:")
print("- runtime_environment.csv")
print("- ALL_DATASETS_BEST_PARAMS.csv")
print("- ALL_DATASETS_CLASS_DISTRIBUTIONS.csv")
print("- ALL_DATASETS_CLASSIFICATION_REPORTS.csv")
print("- ALL_DATASETS_BASELINE_SETTINGS.csv")


# === Aggregate Optuna trial histories across datasets (added) ===
all_trials_frames = []
for dataset_name in CONFIG["DATASET_PATHS"].keys():
    dataset_dir = os.path.join(CONFIG["RESULTS_DIR"], dataset_name)
    trials_csv_path = os.path.join(dataset_dir, f"{dataset_name}_optuna_trials.csv")
...
if all_trials_frames:
    all_trials_summary = pd.concat(all_trials_frames, ignore_index=True)
    all_trials_summary.to_csv(
        os.path.join(CONFIG["RESULTS_DIR"], "INSDN_OPTUNA_TRIALS.csv"),
        index=False
    )
    print("\nCombined Optuna trial history saved: ALL_DATASETS_OPTUNA_TRIALS.csv")

    # ── Combined BO convergence — one panel per dataset ───────────────────────
    datasets_list = list(CONFIG["DATASET_PATHS"].keys())
    n_ds  = len(datasets_list)
    fig, axes = plt.subplots(n_ds, 1, figsize=(9, 5.0 * n_ds), squeeze=False)
    for row_idx, dataset_name in enumerate(datasets_list):
        ax  = axes[row_idx, 0]
        sub = all_trials_summary[all_trials_summary["dataset"] == dataset_name].copy()
        sub = sub[sub["state"] == "COMPLETE"].sort_values("trial_number").reset_index(drop=True)
        ...
        ax.set_ylabel("Composite objective score")
        ax.set_title(
            f"{dataset_name}: Bayesian Optimisation Convergence"
            f" (Optuna TPE, {len(sub)} trials)"
        )
        ax.legend()
    fig.suptitle("Bayesian Optimisation Convergence — All Datasets",
                 fontsize=14, fontweight="bold")
    fig.set_size_inches(9, 5)
    combined_conv_path = os.path.join(CONFIG["RESULTS_DIR"], "INSDN_BO_CONVERGENCE.pdf")
    save_thesis_figure(fig, combined_conv_path)
    plt.close(fig)
    print(f"Saved combined BO convergence figure: {combined_conv_path}")

print("\nAdditional BO artifacts exported:")
print("- <dataset>_optuna_study.pkl   (per dataset)")
print("- <dataset>_optuna_trials.csv  (per dataset)")
print("- <dataset>_bo_convergence.pdf/.png (per dataset)")
print("- ALL_DATASETS_OPTUNA_TRIALS.csv")
print("- ALL_DATASETS_BO_CONVERGENCE.pdf/.png")
