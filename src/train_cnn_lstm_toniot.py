...
# coding: utf-8
"""
CNN-LSTM IDS Training and Evaluation -- ToN-IoT Dataset

Trains and evaluates the Bayesian-optimized CNN-LSTM intrusion detection
model on the ToN-IoT dataset, benchmarked against gradient-boosting and
single-branch (CNN-only, LSTM-only) baselines. Includes Optuna TPE
...
Kaggle notebook (final2-cnn-lstm-ton-iot-only.ipynb) used to produce the results reported in the
...
Usage:
    ...
    Kaggle's /kaggle/input mount), then run:
        python train_cnn_lstm_toniot.py
"""
...
# Cell 3: Reproducibility and Kaggle configuration — ToN-IoT only
SEED = 42
random.seed(SEED)
np.random.seed(SEED)
...
def find_kaggle_csv(expected_filename, fallback_keywords=None):
    fallback_keywords = fallback_keywords or []
    if KAGGLE_INPUT_ROOT.exists():
        exact = sorted(KAGGLE_INPUT_ROOT.rglob(expected_filename))
        if exact:
            return str(exact[0])
        csv_files = sorted(KAGGLE_INPUT_ROOT.rglob("*.csv"))
        kw_matches = [
            p for p in csv_files
            if all(k.lower() in p.name.lower() for k in fallback_keywords)
        ]
        if kw_matches:
            return str(kw_matches[0])
    return f"/kaggle/input/CHANGE_ME/{expected_filename}"


CONFIG = {
    "DATASET_PATHS":  {
        "ToN_IoT": find_kaggle_csv(
            "train_test_network.csv", fallback_keywords=["ton", "iot"]
        )
    },
    "TARGET_COLUMNS": {"ToN_IoT": "type"},
    "RESULTS_DIR": str(KAGGLE_WORKING_ROOT / "thesis_results"),
    "OUTPUT_ZIP":  str(KAGGLE_WORKING_ROOT / "thesis_results.zip"),
    "TEST_SIZE":  0.15,
    "VAL_SIZE":   0.15,
    "TOP_K_FEATURES": 32,
    "TOP_K_FEATURES_BY_DATASET": {"ToN_IoT": 32},
...
_p = CONFIG["DATASET_PATHS"]["ToN_IoT"]
_exists = os.path.exists(_p)
print(f"\nToN-IoT path: {'✓' if _exists else '✗'}  {_p}")
if "CHANGE_ME" in _p:
    print("\n⚠ Attach the ToN-IoT dataset via Add Data and re-run this cell.")

# Cell 4: Verify ToN-IoT file is accessible
if KAGGLE_INPUT_ROOT.exists():
...
_p = CONFIG["DATASET_PATHS"]["ToN_IoT"]
_exists = os.path.exists(_p)
print(f"  {'✓' if _exists else '✗'}  {_p}  "
      f"({os.path.getsize(_p)/1024**2:.1f} MB if exists else '—')")
...
# Cell 13: Run all experiments on ToN-IoT
all_studies = {}

_p = CONFIG["DATASET_PATHS"]["ToN_IoT"]
if "CHANGE_ME" in _p or not os.path.exists(_p):
    print(f"ToN-IoT file not found: {_p}")
    print("Attach the dataset via Add Data and re-run.")
else:
    results_df, reports, study, best_params = run_experiments_for_dataset(
        "ToN_IoT", CONFIG["DATASET_PATHS"]["ToN_IoT"]
    )
    all_studies["ToN_IoT"] = study
    final_results = results_df.copy()
    final_results.to_csv(
        os.path.join(CONFIG["RESULTS_DIR"], "TONIOT_FINAL_RESULTS.csv"), index=False
    )
    print("\nToN-IoT results:")
...
_latex_df.to_latex(
    ...
    caption="ToN-IoT: model performance summary.",
    label="tab:toniot_results",
...
(
    ...
    .set_caption("ToN-IoT results: all models")
    .to_html(_html_path)
)
...
display(
    ...
    .set_caption("ToN-IoT — green = best, highlighted = top performer per metric")
)
...
if best_params_frames:
    ...
    best_params_summary.to_csv(
        os.path.join(CONFIG["RESULTS_DIR"], "TONIOT_BEST_PARAMS.csv"),
        index=False
    )
...
if class_distribution_frames:
    ...
    all_class_distributions.to_csv(
        os.path.join(CONFIG["RESULTS_DIR"], "TONIOT_CLASS_DISTRIBUTIONS.csv"),
        index=False
    )
...
if baseline_setting_frames:
    ...
    all_baseline_settings.to_csv(
        os.path.join(CONFIG["RESULTS_DIR"], "TONIOT_BASELINE_SETTINGS.csv"),
        index=False
    )
...
if classification_report_frames:
    ...
    all_classification_reports.to_csv(
        os.path.join(CONFIG["RESULTS_DIR"], "TONIOT_CLASSIFICATION_REPORTS.csv"),
        index=False
    )
...
if all_trials_frames:
    ...
    all_trials_summary.to_csv(
        os.path.join(CONFIG["RESULTS_DIR"], "TONIOT_OPTUNA_TRIALS.csv"),
        index=False
    )
    ...
    combined_conv_path = os.path.join(CONFIG["RESULTS_DIR"], "TONIOT_BO_CONVERGENCE.pdf")
